import {FileInfo, API, JSCodeshift, Collection, ImportSpecifier, Options} from 'jscodeshift'
...
export default function transformer(fileInfo: FileInfo, api: API, options: Options) {
    ...
    const stringMethods: string[] = ['capitalize', 'camelize', 'classify', 'decamelize', 'dasherize', 'underscore', 'w']
    ...
    root.find(j.ImportDeclaration).forEach((path) => {
        if (Array.isArray(path.value.specifiers)) {
            if (path.value.source.value === '@ember/string') {
                ...
                path.value.specifiers.forEach((specifier) => {
                    if (specifier.local && templateMethods.includes(specifier.local.name)) {
    ...
    root.find(j.CallExpression, {
            ...
            // @ts-expect-error jscodeshfit
            if (path.value.callee.object.type === 'Identifier' && path.value.callee.object.name === '_') {
            ...
            const methodSet = stringMethods.includes(methodName) ? usedStringMethods : usedTemplateMethods
    ...
    const updateOrAddImport = (source: Collection, importSet: Set<string>, moduleName: string) => {
        ...
        if (existingImport.size() > 0) {
            ...
            const existingImports = new Set(existingSpecifiers.map((spec: ImportSpecifier) => {
                if (spec.local) {
            ...
            importSet.forEach((method) => {
                if (!existingImports.has(method)) {
                    existingSpecifiers.push(j.importSpecifier(j.identifier(method), j.identifier(method)))
                }
            })
        } else if (importSet.size > 0) {
            // No import from the module yet, and we have methods to import
            const importDeclaration = j.importDeclaration(
                Array.from(importSet).map((method) => j.importSpecifier(j.identifier(method), j.identifier(method))),
    ...
    root.find(j.VariableDeclarator)
        ...
        .forEach((path) => {
            ...
            if (path.node.id.properties.length === 1) {
                ...
                // @ts-expect-error jscodeshfit
                path.node.id.properties = path.node.id.properties.filter((prop) => prop.key.name !== 'String')
            }
        })
