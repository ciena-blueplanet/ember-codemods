        ...
        .forEach((path) => {
            ...
            if (Array.isArray(path.node.specifiers)) {
                path.node.specifiers.forEach((specifier) => {
                    if (specifier.local && (specifier.local.name === "htmlSafe" || specifier.local.name === "isHTMLSafe")) {
                        newSpecifiers.push(j.importSpecifier(j.identifier(specifier.local.name), j.identifier(specifier.local.name)));
                    }
                });
                ...
                if (newSpecifiers.length > 0) {
                    const existingTemplateImport = j(path.parentPath)
                        .find(j.ImportDeclaration)
                        .filter((templatePath) => templatePath.value.source.value === "@ember/template")
                        .at(0);
                    ...
                    if (existingTemplateImport.size() > 0) {
                        ...
                        const newImport = j.importDeclaration(newSpecifiers, j.literal("@ember/template"));
                        j(path).insertAfter(newImport);
                    }
                    path.node.specifiers = path.node.specifiers.filter(
                        (specifier) => specifier.local && specifier.local.name !== "htmlSafe" && specifier.local.name !== "isHTMLSafe"
                    );
