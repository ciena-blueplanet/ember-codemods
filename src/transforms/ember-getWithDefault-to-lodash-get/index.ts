import {API, FileInfo, ImportSpecifier, JSCodeshift, Options} from 'jscodeshift'
...
export default function transformer(file: FileInfo, api: API, options: Options) {
    ...
    const hasGetWithDefaultUsage = root.find(j.CallExpression, {
        callee: (callee) => 
            (callee.type === 'MemberExpression' && callee.property.type === 'Identifier' && callee.property.name === getwithDefault) ||
    ...
    if (hasGetWithDefaultUsage) {
        ...
        root.find(j.ImportDeclaration).forEach((path) => {
            ...
            if (typeof importSource === 'string') {
                if (['lodash/get', 'lodash-es/get'].includes(importSource)) {
                    const defaultSpecifier = path.value.specifiers?.find((spec) => spec.type === 'ImportDefaultSpecifier')
        ...
        root.find(j.CallExpression, {
            ...
            const newArgs = path.value.callee.type === 'MemberExpression' ? [path.value.callee.object, ...args] : args
        ...
        if (emberObjectImportPath) {
            emberObjectImportPath.value.specifiers =emberObjectImportPath.value.specifiers?.filter(
                (spec: ImportSpecifier) => spec.type !== 'ImportSpecifier' || spec.imported.name !== getwithDefault
            )
