import type { API, JSCodeshift, FileInfo, ObjectProperty, Options } from 'jscodeshift'
...
export default function transformer(fileInfo: FileInfo, api: API, options: Options) {
  ...
  if (fileInfo.path.includes('environment.js')) {
    ...
    const variableDeclarator = root.find(j.VariableDeclarator).filter(path => !!(path.value.init && path.value.init.type === 'ObjectExpression'))
    variableDeclarator.forEach(path => {
      const init = path.value.init
      // @ts-expect-error jscodeshift type error
      const hasLocationType = init && init.properties.some((prop:ObjectProperty) => prop.key.name === 'locationType' && prop.value.value === 'auto')
